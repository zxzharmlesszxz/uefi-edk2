...
EFI_STATUS
EFIAPI
BhyveGopDriverBindingSupported (
  ...
  )
{
  ...
  if (Pci.Hdr.VendorId == 0xFB5D && Pci.Hdr.DeviceId == 0x40FB) {
    ...
    PcdSet32 (PcdSetupVideoHorizontalResolution, Width);
    PcdSet32 (PcdSetupVideoVerticalResolution, Height);
    PcdSet32 (PcdVideoHorizontalResolution, Width);
    PcdSet32 (PcdVideoVerticalResolution, Height);
  }
  ...
  )
{
  ...
  Status = BhyveGopConstructor (Private);
  if (EFI_ERROR (Status)) {
    goto Done;
  }
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    Status = BhyveGopDestructor (Private);
    if (EFI_ERROR (Status)) {
...
///
EFI_DRIVER_BINDING_PROTOCOL gBhyveGopDriverBinding = {
  BhyveGopDriverBindingSupported,
  BhyveGopDriverBindingStart,
  BhyveGopDriverBindingStop,
  0xa,
...
/**
  The user Entry Point for module BhyveGop. The user code starts with this function.
  ...
  )
{
  ...
  Status = EfiLibInstallDriverBindingComponentName2 (
             ImageHandle,
             SystemTable,
             &gBhyveGopDriverBinding,
             ImageHandle,
             &gBhyveGopComponentName,
             &gBhyveGopComponentName2
             );
